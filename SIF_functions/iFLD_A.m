function[F,R]=iFLD_A(wvl,irradiance_data_QEPRO,radiance_data_QEPRO,do_plot,time,path_save_fig)                   
...
E = irradiance_data_QEPRO(pos1A:pos2A);
L = radiance_data_QEPRO(pos1A:pos2A);
...
%% Plots
if do_plot == 1 
    %Interpollated Downwelling radiance (E)
    fig1 = figure('units','normalized','outerposition',[0 0 1 1]);
    subplot(1,2,1)
    plot(wvl,irradiance_data_QEPRO)
    hold on
    plot(wvl_inter,E_INTERPOLATED_1,'r')
    plot(wvl_inter,E_INTERPOLATED_2,'g')
    plot(dwvlNoABS(pks_E1),E_NoABS(pks_E1),'ok')    
    plot(dwvlNoABS(index_E_outside_final),E_NoABS(index_E_outside_final),'og')
    xlim([740 785])
    ylim([0 max(irradiance_data_QEPRO)])
    title('Interpolated Downwelling radiance (E)')
    
    %Interpollated Apparent Reflectance
    subplot(1,2,2)
    plot(wvl_inter,ARHO)
    hold on
    plot(wvl_inter,ARHO_INTERPOLATED_1,'r')
    plot(dwvlNoABS(pks_E1),RappNoABS(pks_E1),'or')    
    xlim([740 785])
    ylim([min(ARHO) max(ARHO)])
    title('Interpolated Apparent Reflectance')
    ...
    name_save_1   = [day{1},'_',hour{1},'iFLD_Interpolated_E_Rapp.png'];
